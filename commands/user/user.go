...
func LFGCommandHandler(session *discordgo.Session, message *discordgo.MessageCreate) {
	...
	session.ChannelMessageSend(newChannel.ID, fmt.Sprintf("<@%s> is looking for %s players for %s - click the :thumbsup: to join in!", message.Author.ID, capacityRaw, gameName)
